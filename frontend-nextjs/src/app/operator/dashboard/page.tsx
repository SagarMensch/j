...
const mandatoryTraining = [
  {
    id: "mod-2",
    module: "Reactor Safety Protocols",
    dueDate: "Oct 15, 2024",
    daysLeft: 5,
    progress: 60,
  },
  {
    id: "mod-3",
    module: "Emergency Response Procedures",
    dueDate: "Nov 1, 2024",
    daysLeft: 21,
    progress: 0,
  },
  {
    id: "mod-4",
    module: "Quality Control Standards",
    dueDate: "Nov 21, 2024",
    daysLeft: 41,
    progress: 0,
  },
...
const safetyAlerts = [
  {
    id: 1,
    title: "Chemical Spill Response Update",
    priority: "high",
    date: "Oct 10, 2024",
  ...
  {
    id: 2,
    title: "New PPE Requirements - Reactor Section",
    priority: "high",
    date: "Oct 9, 2024",
    department: "All",
  },
  {
    id: 3,
    title: "Scheduled Maintenance - Cooling System",
    priority: "medium",
    date: "Oct 8, 2024",
...
export default function OperatorDashboard() {
  ...
  const handleSearch = () => {
    ...
    router.push(`/operator?q=${encodeURIComponent(searchQuery)}`);
  };
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full rounded-[4px] border border-border bg-white px-4 py-3 pr-12 text-foreground placeholder-muted focus:border-secondary focus:outline-none focus:ring-2 focus:ring-secondary/15"
                />
                <button
              ...
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                ...
                {recentSearches.map((search, idx) => (
                  <button
                    ...
                    className="rounded-[4px] border border-border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted transition-colors hover:border-secondary hover:text-secondary"
                  >
                    {search}
                  </button>
                ))}
          ...
          <Card className="!p-0 overflow-hidden">
            <div className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-[8px] border border-border bg-muted-light flex items-center justify-center">
                  <svg
                    className="w-6 h-6 text-primary"
                    fill="none"
                    stroke="currentColor"
                ...
                <div>
                  ...
                  <p className="text-sm text-muted">
                    {sessionResume.moduleName} - Step {sessionResume.stepNumber}{" "}
                    of {sessionResume.totalSteps}
                  </p>
                  <p className="text-xs text-muted">
                    Last accessed: {sessionResume.lastAccessed}
                  </p>
                </div>
              </div>
              <Link
                href={`/operator/training/mod-2?step=${sessionResume.stepNumber}`}
              >
                ...
                <Link key={idx} href={`/operator/training/${item.id}`}>
                  <div className="flex items-center justify-between p-3 bg-muted-light/50 hover:bg-muted-light rounded-lg transition-colors cursor-pointer">
                    ...
                    <div className="text-right ml-4">
                      ...
                      <p
                        className={`text-xs font-medium ${item.daysLeft <= 7 ? "text-danger" : "text-muted"}`}
                      >
        ...
        <Card title="Recent SOPs">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {recentSOPs.map((sop, idx) => (
              <Link
                key={idx}
                href={`/operator?q=${encodeURIComponent(sop.title)}`}
              >
                <div className="p-4 border border-border rounded-[4px] hover:border-secondary hover:bg-secondary/5 transition-all cursor-pointer">
